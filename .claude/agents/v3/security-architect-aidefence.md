...
hooks:
  pre: |
    ...
    AIMDS_RESULT=$(bunx claude-flow@v3alpha security defend --input "$TASK" --mode thorough --json 2>/dev/null)
    ...
    THREAT_PATTERNS=$(bunx claude-flow@v3alpha memory search-patterns "$TASK" --k=10 --min-reward=0.85 --namespace=security_threats 2>/dev/null)
    if [ -n "$THREAT_PATTERNS" ]; then
      ...
      bunx claude-flow@v3alpha memory get-pattern-stats "$TASK" --k=10 --namespace=security_threats 2>/dev/null
    fi

    # ═══════════════════════════════════════════════════════════════
    # PHASE 3: Learn from Past Security Failures
    # ═══════════════════════════════════════════════════════════════
    SECURITY_FAILURES=$(bunx claude-flow@v3alpha memory search-patterns "$TASK" --only-failures --k=5 --namespace=security 2>/dev/null)
    ...
    if [[ "$TASK" == *"auth"* ]] || [[ "$TASK" == *"session"* ]] || [[ "$TASK" == *"inject"* ]] || \
      ...
      bunx claude-flow@v3alpha security cve --check-relevant "$TASK" 2>/dev/null
    fi
  ...
  post: |
    ...
    if [ -n "$SECURITY_SESSION_ID" ]; then
      ...
      BEHAVIOR_RESULT=$(bunx claude-flow@v3alpha security behavior \
      ...
      if [ -n "$BEHAVIOR_RESULT" ]; then
        ...
        if [ "$(echo "$ANOMALY_SCORE > 0.8" | bc 2>/dev/null)" = "1" ]; then
          ...
          bunx claude-flow@v3alpha hooks notify --severity warning \
    ...
    if [ "$SUCCESS" = "true" ] && [ "$(echo "$REWARD > 0.85" | bc 2>/dev/null)" = "1" ]; then
      ...
      bunx claude-flow@v3alpha security learn \
    ...
    if [ "$CRITICAL_COUNT" -gt 0 ]; then
      ...
      bunx claude-flow@v3alpha hooks notify --severity critical \
...
bunx claude-flow@v3alpha security defend --input "<suspicious input>" --mode thorough

# Analyze agent behavior
bunx claude-flow@v3alpha security behavior --agent <agent-id> --window 1h

# Verify LTL security policy
bunx claude-flow@v3alpha security policy --agent <agent-id> --formula "G(edit -> F(review))"

# Record successful mitigation for meta-learning
bunx claude-flow@v3alpha security learn --threat-type prompt_injection --strategy sanitize --effectiveness 0.95
```
