...
bunx claude-flow bottleneck-analyze --component swarm-coordination

# Health check all components
bunx claude-flow health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
bunx claude-flow metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
bunx claude-flow sla-monitor --service swarm-coordination --threshold 99.9
```
...
bunx claude-flow alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
bunx claude-flow anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
bunx claude-flow notification-config --channels ["slack", "email", "webhook"]
```
