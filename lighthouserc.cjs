...
const previewHost = process.env.LIGHTHOUSE_BASE_URL || 'http://127.0.0.1:4321';

module.exports = {
  ci: {
    collect: {
      // For SSR Astro apps, we need to start the server first
      startServerCommand: 'PORTLESS=0 bun run preview:raw',
      ...
      url: [
        `${previewHost}/`, // Homepage (most critical)
        `${previewHost}/blog`, // Blog listing page
        `${previewHost}/assine`, // Subscription/conversion page
        `${previewHost}/parcerias`, // Partnerships page
        `${previewHost}/jng/jobs`, // Jobs listing (member area)
        `${previewHost}/ebook`, // Marketing page
        `${previewHost}/politicas`, // Policies page (simple content)
      ],
