...
✅ **Assets Migration**: All assets copied from `src-legacy/cdn-assets/` to
`public/cdn-assets/`
...
1. **general_style duplication**: Some pages may have duplicate `general_style`
   divs (BaseLayout already includes it)
   - Fix: Remove duplicate `general_style` divs from migrated pages during
     visual comparison
